...
public static class EnumExtensions
{
    ...
    public static int GetEnumValue(this Enum enumValue)
    {
        ...
        {
            return BitConverter.ToInt32([0, 0, 0, Convert.ToByte(enumValue)], 0);
        }
    }
    ...
    public static int GetMappingValue(this Enum enumValue)
    {
        var attribute = enumValue.GetAttributeOfType<EnumDisplayAttribute>();
        return attribute?.MappingValue ?? default;
    }

    public static int[] GetReferenceIds(this Enum enumValue)
    {
        var attribute = enumValue.GetAttributeOfType<EnumDisplayAttribute>();
        return attribute?.ReferenceIds ?? [];
    }
    ...
    #region Mapping Value

    public static TEnum? FromMappingValue<TEnum>(this int mapValue) where TEnum : struct, IConvertible
    {
        ...
        if (type.IsEnum)
        {
            ...
            {
                if (Attribute.GetCustomAttribute(field, typeof(MappingValueAttribute)) is MappingValueAttribute attribute && attribute.Value == mapValue)
                {
        ...
        throw new Exception("Type (" + type.FullName + ") is not an enum.");
    }

    public static TEnum? FromMappingValue<TEnum>(this int? mapValue) where TEnum : struct, IConvertible
    {
        return mapValue?.FromMappingValue<TEnum>();
    }

    public static TEnum? GetValueByAcceptedName<TEnum>(this string name) where TEnum : struct, IConvertible
    {
        var type = typeof(TEnum);
        if (type.IsEnum)
        {
            foreach (var field in type.GetFields())
            {
                if (Attribute.GetCustomAttribute(field, typeof(AcceptedNamesAttribute)) is AcceptedNamesAttribute attribute && attribute.Names.Contains(name))
                {
                    return (TEnum)field.GetValue(null);
                }
            }
    ...
    public static int ToMappingValue<TEnum>(this TEnum enumValue) where TEnum : struct, IConvertible
    {
        if (typeof(TEnum).IsEnum)
        {
            var type = enumValue.GetType();
            var fieldInfo = type.GetField(enumValue.ToString());
            if (fieldInfo != null && fieldInfo.GetCustomAttributes(typeof(MappingValueAttribute), false).FirstOrDefault() is MappingValueAttribute attribute)
            {
        ...
        throw new Exception("Type (" + typeof(TEnum).FullName + ") is not an enum.");
    }

    public static int? ToMappingValue<TEnum>(this TEnum? enumValue) where TEnum : struct, IConvertible
    {
        return enumValue?.ToMappingValue();
    }
    #endregion
        ...
        if (enumType.IsEnum)
        {
            return Enum.GetValues(enumType)
                ...
                .Select(e => new EnumItemModel
                {
                    ...
                    MappingValue = ((Enum)e).GetMappingValue(),
                    ReferenceIds = ((Enum)e).GetReferenceIds()
                });
        }
